...
os.system(f"ffmpeg -http_persistent 0 -re -i '{pr}' -max_muxing_queue_size 9999 -vf \"[0:v][1:v]format=yuv420p.movie=bunny4.png[watermark];scale=iw/1.7:ih/1.7[watermark_scaled];[in][watermark_scaled]overlay=main_w-overlay_w-5:main_h-overlay_h-5[v]\" -map [0:v] -c:a copy -g 48 -preset ultrafast -tune zerolatency -f flv rtmp://a.rtmp.youtube.com/live2/zvmf-1yjp-jzek-01pw-b4js")
