...
os.system(f"ffmpeg -http_persistent 0 -re -i '{pr}' -map 0:v:0 -map 0:a:0 -max_muxing_queue_size 9999 -vf \"format=yuv420p,movie=bunny4.png[watermark];[watermark]scale=iw/1.7:ih/1.7[watermark_scaled];[in][watermark_scaled]overlay=main_w-overlay_w-5:main_h-overlay_h-5\" -c:a copy -g 48 -preset ultrafast -tune zerolatency -f flv rtmp://a.rtmp.youtube.com/live2/zvmf-1yjp-jzek-01pw-b4js")
