...
#short -ß
os.system(f"ffmpeg -http_persistent 0 -drop_pkts_on_overflow 1 -attempt_recovery 1 -recover_any_error 1 -re -ss 00:00:12 -i '{pr}' -max_muxing_queue_size 9999 -threads 4 -vf \"format=yuv420p,drawtext=fontfile=_.ttf:text='F B':fontcolor=white:fontsize=130:line_spacing=32:bordercolor=black:borderw=10:box=0:boxcolor=black@0.1:boxborderw=80:x=w-tw-20:y=h-th-20,transpose=1\" -c:v libx264 -g 48 -b:v 13000k -c:a aac -preset ultrafast -tune zerolatency -f flv rtmp://a.rtmp.youtube.com/live2/zvmf-1yjp-jzek-01pw-b4js")
